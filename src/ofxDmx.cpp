    ...
    if( needsUpdate || force ) {
        ...
        for( unsigned int i = 0; i < universes; i++ ) {
            ...
            if( i == 0 )
                ...
                copy( levels2.begin(), levels2.end(), packet.begin() + 5 );
...
bool ofxDmx::badChannel( unsigned int channel )
{
    if( channel > levels.size() ) {
        ofLogError() << "Channel " + ofToString( channel ) + " is out of bounds. Only " + ofToString( levels.size() )
